...
export default function CancelledOrdersPage() {
    ...
    useEffect(() => {
        ...
        async function loadOrders() {
            ...
            if (error) {
                setError(error.message || "Failed to load cancelled orders.");
                setOrders([]);
                setLoading(false);
                return;
            }
    ...
    const headingText = useMemo(() => {
        if (loading) return "Loading cancelled orders...";
        if (error) return "We couldn’t load your cancelled orders right now.";
        if (orders.length === 0) return "No cancelled orders found yet.";
        return `${orders.length} cancelled order${orders.length === 1 ? "" : "s"} found`;
        ...
        <main className="min-h-screen bg-[#fff4f8]">
            ...
            <section className="mx-auto max-w-7xl px-6 py-12">
                <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
                    <div>
                        ...
                        <p className="mt-3 text-sm text-black/60">
                            Live cancelled orders synced from your Loomeira database.
                        </p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="mt-5 rounded-[24px] border border-[#f2cddd] bg-white px-6 py-10 text-sm text-black/55 shadow-sm">
                        No cancelled orders are available yet.
                    </div>
                ) : (
